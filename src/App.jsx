...
import { CATEGORIES, SEVERITIES, DEFAULT_THRESHOLD, categoryColors } from "./data/taxonomy.js";
import { calcScore, penaltyPer1000 } from "./lib/scoring.js";
import { buildSummaryData, buildErrorData } from "./lib/report.js";
import { css } from "./theme.js";
import Badge from "./components/Badge.jsx";
import Spots from "./components/Spots.jsx";

const STORAGE_KEY = "mqm_scorer_session";
const EMPTY_META = { project: "", langPair: "", wordCount: "", reviewer: "", file: "" };
const EMPTY_FORM = { source: "", target: "", revised: "", category: "Terminology", subcategory: CATEGORIES["Terminology"][0], severity: "Major", comment: "" };
...
export default function MQMScorer() {
  const [meta, setMeta] = useState(() => ({ ...EMPTY_META, ...loadSession().meta }));
  const [errors, setErrors] = useState(() => loadSession().errors ?? []);
  const [threshold, setThreshold] = useState(() => loadSession().threshold ?? DEFAULT_THRESHOLD);
  const [form, setForm] = useState(EMPTY_FORM);

  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ meta, errors, threshold }));
    } catch (e) {
      console.warn("Could not save session:", e);
    }
  }, [meta, errors, threshold]);

  const fileWordCount = parseInt(meta.wordCount) || 0;
  const score = calcScore(errors, fileWordCount);
  const npt = penaltyPer1000(errors, fileWordCount);
  ...
  const exportExcel = () => {
    ...
    const summaryData = buildSummaryData({ meta, errors, wordCount: fileWordCount, score, pass, threshold, date });
    const errorData = buildErrorData(errors);

    const wb = XLSX.utils.book_new();
    const wsSummary = XLSX.utils.aoa_to_sheet(summaryData);
    const wsErrors = XLSX.utils.aoa_to_sheet(errorData);

    wsSummary["!cols"] = [{ wch: 24 }, { wch: 16 }, { wch: 12 }, { wch: 12 }, { wch: 10 }, { wch: 12 }, { wch: 14 }];
    wsErrors["!cols"] = [{ wch: 4 }, { wch: 30 }, { wch: 30 }, { wch: 30 }, { wch: 22 }, { wch: 28 }, { wch: 10 }, { wch: 8 }, { wch: 30 }];
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: 28, display: "flex", alignItems: "flex-end", gap: 16 }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", color: "#7a6040", fontSize: 13, ...css.sans }}>MQM CORE typology evaluation with weighted error scoring</p>
          </div>
          <div style={{ fontSize: 28, marginBottom: 4 }}>🦒</div>
        </div>

        <div style={css.card}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#c8a870", marginBottom: 14, ...css.sans }}>Project Info</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            {[["project", "Project name"], ["langPair", "Language pair"], ["file", "File name"], ["reviewer", "Reviewer"], ["wordCount", "Total word count of file"]].map(([key, lbl]) => (
              <div key={key}>
            ...
            <div>
              <label style={css.label}>Pass threshold (%)</label>
              <input style={css.input} type="number" min="0" max="100" step="0.5" value={threshold} onChange={e => setThreshold(parseFloat(e.target.value) || 0)} />
            </div>
          </div>
        </div>

        <div style={css.card}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#c8a870", marginBottom: 14, ...css.sans }}>Add Error</div>
        ...
        {errors.length > 0 && (
          <div style={css.card}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#c8a870", ...css.sans }}>Error Log ({errors.length})</div>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                      ...
                      <span style={{ fontSize: 12, color: categoryColors[e.category], fontWeight: 600, ...css.sans }}>{e.category}</span>
                      <span style={{ fontSize: 12, color: "#7a6040", ...css.sans }}>{e.subcategory}</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                      {[["Source", e.source, "#888"], ["Target", e.target, "#c8b98a"], ["Revised", e.revised, "#a8c878"]].map(([lbl, val, col]) => val ? (
                        <div key={lbl}>
                          <div style={{ fontSize: 10, color: "#7a6040", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2, ...css.sans }}>{lbl}</div>
                          <div style={{ fontSize: 12, color: col, ...css.sans }}>{val}</div>
                        </div>
                      ) : null)}
                    </div>
                    {e.comment && <div style={{ fontSize: 12, color: "#7a6040", marginTop: 6, ...css.sans }}>{e.comment}</div>}
                  </div>
        ...
        {errors.length > 0 && (
          <div style={css.card}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#c8a870", ...css.sans }}>Quality Report</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 20 }}>
              {[
                ...
                { label: "Penalty / 1000 words", val: npt ?? "—", color: "#d4956a" },
                ...
                <div key={label} style={{ background: "#1a1510", border: "1px solid #3a2e1e", borderRadius: 10, padding: "14px 12px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#7a6040", fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", marginTop: 2, ...css.sans }}>{label}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 12, color: "#7a6040", ...css.sans }}>0</span>
                <span style={{ fontSize: 12, color: "#7a6040", ...css.sans }}>Threshold: {threshold}</span>
                <span style={{ fontSize: 12, color: "#7a6040", ...css.sans }}>100</span>
              </div>
              ...
              {SEVERITIES.map(s => {
                ...
                return (
                  <div key={s.label} style={{ background: s.bg, border: `1px solid ${s.color}33`, borderRadius: 8, padding: "10px 12px" }}>
                    ...
                    <div style={{ fontSize: 11, color: s.color, fontWeight: 600, ...css.sans }}>{s.label}</div>
                    <div style={{ fontSize: 11, color: "#7a6040", ...css.sans }}>-{count * s.points} pts</div>
                  </div>
                );
              })}
            </div>
            ...
            {groupedErrors.length > 0 && (
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: "#7a6040", marginBottom: 10, ...css.sans }}>Errors by category</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {groupedErrors.map(({ cat, items }) => (
                    <div key={cat} style={{ background: "#1a1510", border: "1px solid #3a2e1e", borderRadius: 8, padding: "8px 14px", display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      <span style={{ fontSize: 12, color: "#7a6040", ...css.sans }}>{cat}</span>
                    </div>
                  ))}
                </div>
              </div>
